...
> ⚠️ **범위: RTSP 수신/관리 + 트리거 + BurstPackage + AI 인터페이스 + OpenCV 전처리 + MediaPipe 특징추출 + YOLO 객체추출 + Facts 통합 + 규칙 판정(RuleDecision)까지만.**
> **Supabase(DB) 저장 / 대시보드 표시 / 학생 상태 변경 / 알림 발송은 구현하지 않습니다.**
> Rule Engine v0.1 에서 **처음으로 activity 가 UNKNOWN 이 아닌 값**이 나올 수 있지만, 결과는 **데이터(RuleDecision)일 뿐** 아직 어디에도 저장·표시되지 않습니다.
