...
> ⚠️ **범위: RTSP 수신/관리 + 트리거 + BurstPackage + AI 인터페이스 + OpenCV 전처리 + MediaPipe 특징추출 + YOLO 객체추출 + Facts 통합 + 규칙 판정 + 판정결과 저장까지만.**
> **대시보드 표시 / 학생 상태 자동 변경 / 알림 / 벌점 / 출결 / 이용권 / 영상·이미지 저장은 구현하지 않습니다.**
> AI Decision Storage 는 **RuleDecision(텍스트/JSON)만** 안전하게 저장합니다 — 저장해도 **학생 상태는 바뀌지 않습니다.**
