# Solomon Camera Core / CameraManager / Scheduler / Orchestrator v0.1

TP-Link **VIGI C420I** 카메라의 RTSP 스트림을 수신·관리하고, 교시 기반 분석 트리거를 생성해
프레임 묶음(BurstPackage)을 만드는 모듈군.
...
- **Orchestrator Engine v0.1** — 트리거→큐→워커→**BurstPackage 생성** 중앙 제어 (`OrchestratorEngine`)

> ⚠️ **범위: RTSP 수신/관리 + 교시 트리거 + BurstPackage 생성까지만.**
