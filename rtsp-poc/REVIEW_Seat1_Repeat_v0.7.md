...
- 작성일: 2026-07-02
- 개정: **v6 — v0.8 P1 drift-aware scheduler 현장 60분 최종 PASS 반영**
- 상태: **STAGE 2 현장 검증 통과(2026-07-02)** — Seat1 real RTSP 60분 save+preview **32/32 insert · 32/32 preview 성공**, 누적 12→20→52 정합 확인. Python 1차 커밋 `f218c8f` + 프론트 2차 커밋 `0ea218a`(브랜치 `feat/v0.7-seat1-repeat`, origin push 완료) · 테스트 통과(py 34/158, FE 51) · build OK. **CTO 리뷰 파일: `cto_review_v0.7_seat1_field_verification.md`.**
  - ⚠️ v0.7 STAGE 2 시점에는 **1분 주기 정확도 완전 검증은 아니었다**(§9 known observations — tick 지연 관찰).
  - ✅ **v0.8 P1 업데이트(2026-07-02): drift-aware scheduler 현장 60분 최종 PASS.** total_runs=61·saved=61·previews=61, drift/late=0.0, scheduler_overrun_count=0 으로 1분 cadence 수렴 확인 → **§11 참고**. **v0.8 P1: COMPLETE.**
...
1. **tick 지연 원인 분석** — §9의 단계별 duration logging 추가(run_once/캡처/트랜스코딩/카메라 open·close/sleep drift 각 단계 소요시간 계측). 원인은 아직 미확정이므로 계측 후 판단. → **v0.8 계측 구현 완료**(`seat1_e2e_test.py --perf-log` 기본 켜짐 + `perf_summary`, `[perf Seat1]` 한 줄 로그). 상세: `cto_handoff_v0.8_tick_perf_logging.md`. → ✅ **v0.8 P1 완료**: 현장 60분 재테스트에서 `perf_summary` 분석 결과 작업시간 avg 19.2초·drift 0.0 확인. **§11 참고.**
2. **1분 주기 정확도 개선 여부 결정** — 고정 sleep(interval) → "다음 정각 정렬(drift 보정)" 스케줄로 바꿀지, 아니면 "지연 허용 + 저장/preview 무결성 우선" 현행 유지로 확정할지 CTO 결정. → ✅ **v0.8 P1 완료**: drift-aware scheduler 채택·구현·현장 60분 PASS. total_runs 46(fixed sleep) → 61(drift-aware)로 wall-clock 60초 cadence 수렴. **§11 참고.**
...
## 11. v0.8 P1 — Drift-aware Scheduler 현장 최종 판정 (2026-07-02)

§9의 tick 지연 관찰과 §10-1·§10-2 후속 과제를 해소하기 위해 **drift-aware scheduler**를 도입하고
현장 20분 → 60분 controlled field test로 검증했다.

### 11-1. 최종 판정

- **v0.8 P1 drift-aware scheduler: COMPLETE**
- **Seat1 60분 controlled field cadence: PASS**
- 근거: fixed sleep 구조 total_runs=46 → drift-aware 적용 후 total_runs=61 로 wall-clock 60초 cadence 수렴.

### 11-2. 60분 Runtime / Perf 지표

- 테스트 명령: `python seat1_e2e_test.py --duration 60 --interval 60 --save --preview --seat Seat1`
- total_runs = 61
- saved = 61
...
- avg_total_tick_duration ≈ 19.2초 (작업시간), sleep ≈ 40초대로 자동 조정

### 11-3. Verify Accumulation (읽기 전용)

- 명령: `python seat1_e2e_test.py --verify-accumulation --seat Seat1 --limit 100`
- Supabase GET **HTTP/2 200 OK**
- **read-only verify PASS** (insert/update/delete 없음)
- ⚠️ **total_rows=100 은 `--limit 100` 조회 결과이며 전체 row count 가 아님.**

### 11-4. NO-GO (유지 — 이번 PASS로 승인되지 않음)
...
- 20분 field result: `cto_field_result_v0.8_p1_20min_drift_scheduler.md`
- 60분 field result: `cto_field_result_v0.8_p1_60min_drift_scheduler.md`
